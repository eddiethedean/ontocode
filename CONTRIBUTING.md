...
This runs rustfmt, `./scripts/check-doc-versions.sh`, clippy, workspace tests, MSRV (1.88), CLI smoke, release build, LSP smoke tests, webview-ui tests, extension build/tests, `cargo audit`, crate packaging dry-run, mkdocs strict build, and VS Code extension e2e. Expect 30+ minutes on a cold build. **Not required for docs-only PRs** — see the [testing matrix](docs/guides/testing-matrix.md).

## Pull requests

1. Fork and branch from `main`.
2. Keep changes focused; match existing style and naming.
3. Run the **scoped** checks from the [testing matrix](docs/guides/testing-matrix.md) (docs-only PRs do not need `cargo test --workspace`).
4. Update docs when behavior or public API changes (`README.md`, `docs/`, `CHANGELOG.md` as appropriate). On release, follow the checklist in [docs/releasing.md](docs/releasing.md). Keep root/`docs/` mirrors in sync where the mirror policy applies.
