...
**UI phases:** **0**, **1**, partial **3** (schema browser), partial **9** (PR summary engine). Milestones: [Product Roadmap 2.0 phases 0–1](docs/ui/PRODUCT_ROADMAP_2.0.md#phase-0).
...
| **OntoUI** | **[0]** Extension UX audit; centralized webview communication; WorkspaceHost adapter; design tokens + reusable components ([DESIGN_TOKENS](docs/ui/DESIGN_TOKENS.md), [COMPONENT_LIBRARY](docs/ui/COMPONENT_LIBRARY.md)). **[1]** WorkspaceStore + Current Focus; semantic navigation history; explorer ↔ inspector sync; core event bus ([STATE_MANAGEMENT](docs/ui/STATE_MANAGEMENT.md), [WORKSPACE_MODEL](docs/ui/WORKSPACE_MODEL.md), [EVENT_SEQUENCE_DIAGRAMS](docs/ui/EVENT_SEQUENCE_DIAGRAMS.md)). **[3]** Query schema browser ([QUERY_WORKBENCH](docs/ui/QUERY_WORKBENCH.md)). Supporting: accessibility ([ACCESSIBILITY_SPEC](docs/ui/ACCESSIBILITY_SPEC.md)), visual design system ([VISUAL_DESIGN_SYSTEM](docs/ui/VISUAL_DESIGN_SYSTEM.md)), webview integration tests ([TESTING_STRATEGY](docs/ui/TESTING_STRATEGY.md)) |
...
**Dependencies:** `sqlparser` ([ADR-0011](docs/design/adr/0011-use-sqlparser-for-sql.md)); evaluate DataFusion only if virtual-table joins are insufficient; [cursor-prompts/](docs/cursor-prompts/README.md) 01–05, 08, 13
