...
pub fn read_file_capped(path: &Path, max_bytes: u64) -> Result<Vec<u8>> {
    let mut file = File::open(path).map_err(|e| OntoIndexError::Scanner(e.to_string()))?;
    let metadata = file.metadata().map_err(|e| OntoIndexError::Scanner(e.to_string()))?;
    ...
    loop {
        let n = file.read(&mut chunk).map_err(|e| OntoIndexError::Scanner(e.to_string()))?;
        if n == 0 {
            break;
        }
...
pub fn read_to_string_capped(path: &Path, max_bytes: u64) -> Result<String> {
    ...
    String::from_utf8(bytes)
        .map_err(|e| OntoIndexError::Scanner(format!("invalid UTF-8 in {}: {e}", path.display())))
}
