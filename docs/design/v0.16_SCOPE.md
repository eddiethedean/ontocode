...
This document is the **v0.16.0 release scope** for OntoCode (VS Code extension) and OntoCore (Rust engine).
...
- **Layout persistence (minimal but real)**
  - Restore key OntoCode panels across sessions using VS Code’s supported persistence hooks (panel serializers) where possible.
  - Provide “Reset OntoCode layout” command to return to a known good default.
