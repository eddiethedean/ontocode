...
1. Open [GitHub Releases](https://github.com/eddiethedean/ontocode/releases) for the latest **v0.21.x** tag.
2. **For CLI on Linux x64:** download `ontocore-v<version>-x86_64-unknown-linux-gnu.tar.gz`.
3. **For VS Code (any supported OS):** download `ontocode-v0.21.0.vsix` (pattern: `ontocode-v<version>.vsix`) — see [vscode-install.md](vscode-install.md).
4. Verify with `SHA256SUMS` — see [release-integrity.md](release-integrity.md).
5. Extract and run (Linux example; replace `0.21.0` with your tag):
...
For VS Code, install the `ontocode-v*.vsix` from the same release (example: `ontocode-v0.21.0.vsix`).

### Air-gapped / offline install

1. Download `ontocode-v0.21.0.vsix` (pattern: `ontocode-v<version>.vsix`) and optional Linux CLI tarball from [GitHub Releases](https://github.com/eddiethedean/ontocode/releases) on a connected machine.
