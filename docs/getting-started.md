...
**Version pinning:** Use `--locked` for reproducible installs from crates.io (recommended). Pin an exact release in CI with `cargo install ontocore-cli --locked --version 0.18.2` — see [API stability](guides/api-stability.md) and [release integrity](release-integrity.md).
...
1. Open [GitHub Releases](https://github.com/eddiethedean/ontocode/releases) for the latest **v0.19.x** tag.
...
5. Extract and run (Linux example; replace `0.18.2` with your tag):
