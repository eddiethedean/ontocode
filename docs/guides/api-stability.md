...
OntoCode and OntoCore are **pre-1.0**. Published crates use **0.20.x** semver, but minor releases may add or change APIs until v1.0.0. This page tiers surfaces by stability so integrators can assess risk.
...
|------|---------|-----------|-------|
| **A — Stable enough for CI** | `ontocore validate`, `query`, `sparql`, `classify`, `diff`, `docs`, `patch`, `robot`, `plugins`, `workflow` CLI | High for **commands and exit codes** | Pin with `cargo install ontocore-cli --locked --version 0.20.0`. Exit codes documented in [workspace limits](../workspace-limits.md). |
...
**VS Code:** install OntoCode **0.20.0** from Marketplace, Open VSX, or a release VSIX — the bundled `ontocore-lsp` matches the extension version.
