...
OntoCode and OntoCore are **pre-1.0**. Published crates use **0.22.x** on crates.io (latest tagged). Minor releases may add or change APIs until v1.0.0.
...
|------|---------|-----------|-------|
| **A — Stable enough for CI** | `ontocore validate`, `query`, `sparql`, `classify`, `diff`, `docs`, `patch`, `robot`, `plugins`, `workflow` CLI | High for **commands and exit codes** | Pin with `cargo install ontocore-cli --locked --version 0.22.0`. Exit codes documented in [workspace limits](../workspace-limits.md). |
...
**VS Code:** install OntoCode **0.22.0** (latest tagged) from Marketplace, Open VSX, or a release VSIX — the bundled `ontocore-lsp` matches the extension version.
