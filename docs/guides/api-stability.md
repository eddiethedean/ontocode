...
|------|---------|-----------|-------|
| **A — Stable enough for CI** | `ontocore validate`, `query`, `sparql`, `classify`, `realize`, `check-instance`, `diff`, `docs`, `patch`, `robot`, `plugins`, `workflow` CLI | High for **commands and exit codes** | Pin with `cargo install ontocore-cli --locked --version 0.23.0`. Exit codes documented in [workspace limits](../workspace-limits.md). No `ontocore swrl` CLI — SWRL via IDE/LSP/patches. |
