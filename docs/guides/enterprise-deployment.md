...
1. On a connected staging machine, download from [GitHub Releases](https://github.com/eddiethedean/ontocode/releases) for version **v0.24.0** (or your pinned version):
   - `ontocode-v0.24.0.vsix` (pattern: `ontocode-v<version>.vsix`)
   - `SHA256SUMS`
   - `NOTICES`
...
|----------|----------------|
| Version pin | Standardize on one release tag (e.g. `v0.24.0`) across VSIX and CLI |
...
|----------|-------------------|--------------|
| Linux x64 | **Yes** | — |
| macOS | **No** | `cargo install ontocore-cli --locked --version 0.24.0` (requires Rust on agent) |
