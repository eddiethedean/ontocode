...
4. Communicate **single-root workspace** rule — only first folder indexed ([FAQ](../faq.md))

### Option B — Offline / air-gapped (recommended for regulated envs)

1. On a connected staging machine, download from [GitHub Releases](https://github.com/eddiethedean/ontocode/releases) for version **v0.7.0** (or your pinned version):
...
|----------|----------------|
| Version pin | Standardize on one release tag (e.g. `v0.7.0`) across VSIX and CLI |
...
|----------|-------------------|--------------|
| Linux x64 | **Yes** | — |
| macOS | **No** | `cargo install ontoindex-cli --locked --version 0.7.0` (requires Rust on agent) |
