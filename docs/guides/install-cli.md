...
| Custom `ontocode.lspPath` without compiling | Prebuilt **`ontocore-lsp-*`** archives on [GitHub Releases](https://github.com/eddiethedean/ontocode/releases/tag/v0.26.2) (macOS, Windows, Linux x64/arm64) — set `ontocode.lspPath` to the extracted binary (trusted workspace) |
...
| Wrong version after install | Re-run with `--version 0.26.2 --locked`; check `ontocore --version` |
