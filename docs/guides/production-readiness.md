...
OntoCode v0.22 is **not** documented as a general-availability replacement for Protégé for every advanced OWL 2 DL workflow (e.g. full Manchester axiom coverage for all formats; byte-identical XML layout). RDF/XML and OWL/XML write-back **ship** in v0.21 (semantic re-serialize).
