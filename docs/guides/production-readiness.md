...
This page states what OntoCode / OntoCore **v0.17.0** is appropriate for in production-like environments. It is not legal advice and does not replace your organization's risk review.
...
|-------|---------|---------|
| **Pre-1.0** | **0.17.x (current)** | Active development. Library APIs, LSP JSON, and SQL virtual table columns may change between minor releases until [v1.0](../design/v1.0_BACKLOG.md). |
...
Pin CLI version in CI: release binary with `VERSION=0.17.0` or `cargo install ontocore-cli --locked --version 0.17.0`.
...
| Supported versions | 0.17.x ([security policy](../security.md)) |
