...
| Exact Protégé DL Query tab / Manchester fidelity expectations | OntoCode ships DL Query (v0.24+) with [documented honesty limits](dl-query.md) — dual-check critical queries if you require Protégé+HermiT identity |
...
| Workflows that depend on Protégé-specific plugins | Not replicated; OntoCode uses subprocess SDK 1.0 — not a Protégé plugin host |
