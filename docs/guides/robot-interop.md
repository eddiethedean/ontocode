...
OntoCore v0.18.2 wraps the [ROBOT](http://robot.obolibrary.org/) CLI for validate, merge, and report workflows. ROBOT runs as an **external Java process** — OntoCore does not embed a JVM.
...
| **OntoCore 0.18.2+** | `ontocore robot` subcommand or LSP `ontocore/runRobot` |
...
- name: Install OntoCore
  run: cargo install ontocore-cli --locked --version 0.18.2
