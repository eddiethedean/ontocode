...
Canonical pin: **`0.26.0`** ([TAGGED_RELEASE](TAGGED_RELEASE)).
...
| Release VSIX (`ontocode-v0.26.0.vsix`) | Same | No |
...
See [CI integration](ci-integration.md) — download `ontocore-v0.26.0-x86_64-unknown-linux-gnu.tar.gz`, verify `SHA256SUMS`, run `validate` / `classify`.
