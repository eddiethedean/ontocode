...
- New: `ontocore robot validate|merge|report`
- SQL `entities` / entity tables expose optional **`obo_id`** column for OBO-indexed terms.
- Pin CI: `cargo install ontocore-cli --locked --version 0.7.0` or release `VERSION=0.7.0`.

## LSP

New custom methods (additive):

- `ontocore/getGraph` — graph export for webviews
- `ontocore/runRobot` — ROBOT CLI wrapper
