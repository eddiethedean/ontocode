...
1. Pin the latest tagged release from [docs/TAGGED_RELEASE](https://github.com/eddiethedean/ontocode/blob/main/docs/TAGGED_RELEASE) (currently **0.26.0**):

   ```bash
   cargo install ontocore-cli --locked --version 0.26.0
   ```

2. For release tarballs, download assets from the matching tag (e.g. `v0.26.0`). Prefer the tagged release over docs that may preview a future minor on `main`.
...
| `cargo install` network / crates.io errors | Retry with `--locked`; pin `--version 0.26.0` in CI |
...
| Corporate Marketplace blocked / lag | Install `ontocode-v0.26.0.vsix` from [GitHub Releases](https://github.com/eddiethedean/ontocode/releases) — [VS Code install](vscode-install.md) |
