...
for platform in "${platforms[@]}"; do
  ...
  if [[ "$platform" == win32-* ]]; then
    asset="ontocore-lsp-${TAG}-${platform}.zip"
    tmp="$(mktemp -d)"
    gh release download "$TAG" --repo "$REPO" --pattern "$asset" --dir "$tmp"
    unzip -q "$tmp/$asset" -d "$tmp/out"
    cp "$tmp/out/ontocore-lsp-${TAG}-${platform}.exe" "$dest/ontocore-lsp.exe"
    rm -rf "$tmp"
  else
    asset="ontocore-lsp-${TAG}-${platform}.tar.gz"
    tmp="$(mktemp -d)"
    gh release download "$TAG" --repo "$REPO" --pattern "$asset" --dir "$tmp"
    tar -xzf "$tmp/$asset" -C "$tmp"
    cp "$tmp/ontocore-lsp-${TAG}-${platform}" "$dest/ontocore-lsp"
