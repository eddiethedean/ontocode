...
"#;
    ...
    assert!(
        text.contains("subPropertyOf"),
        "expected subPropertyOf in file: {text}"
    );
    assert!(
    ...
    assert!(hit, "expected d ⊑ e in catalog or text; axioms={:?} text={text}", catalog.data().axioms);
}
...
fn hierarchy_tabbed_edges_apply_as_subclassof() {
    ...
    assert_parent_of(
        &catalog,
        &format!("{ns_base}ChildA"),
        &format!("{ns_base}Parent"),
    );
    assert_parent_of(
        &catalog,
        &format!("{ns_base}GrandChild"),
        &format!("{ns_base}ChildB"),
    );
}
